...
pre-commit-lib.run {
  ...
  hooks = {
    ...
    a-dotnet-lint-Beryllium = {
      enable = true;
      name = "Lint .NET 'Beryllium' Project";
      description = "Run formatter for .NET Project 'Beryllium'";
      entry = "${packages.dotnet}/bin/dotnet format style --no-restore --severity info --verify-no-changes -v d ./Beryllium/Beryllium.csproj";
      language = "system";
